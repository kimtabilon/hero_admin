...
final TextEditingController OptionIDController = TextEditingController();
  ...
  DataRow _buildListItem(BuildContext context, DocumentSnapshot snapshot) {
    return DataRow(
        onSelectChanged: (bool selected) async {
          if (selected) {
            OptionIDController.text = snapshot.id;
            nameController.text = snapshot.data()['name'];
            descriptionController.text = snapshot.data()['description'];
            ServiceIDController.text = snapshot.data()['service_id'];
        ...
        onPressed: () async {
          ...
          if (_formKey.currentState.validate()) {
            ...
            try {
              ...
              if(action == "add"){
                await db.collection('service_option').add(
                    {
                ...
                Toast.show("Service Option Added Successfully", context, duration: Toast.LENGTH_LONG, gravity:  Toast.BOTTOM);

              }else{
                await db.collection('service_option').doc(OptionIDController.text).update(
                    {
                      'name': nameController.text.trim(),
                      'description': descriptionController.text,
                      'icon': int.parse(iconController.text),
                ...
                //     DialogType.SUCCES, Icons.check_circle);
                Toast.show("Service Option Updated Successfully", context, duration: Toast.LENGTH_LONG, gravity:  Toast.BOTTOM);
                ...
                builder: (context,snapshot){
                  if(!snapshot.hasData){
                    ...
                    for(int i=0;i<snapshot.data.documents.length;i++){
                      ...
                      if(i == 0 && ServiceIDController.text.isEmpty){
                        ServiceIDController.text = snap.id;
                      }
                    }
                    return Container(
                      padding: EdgeInsets.symmetric(horizontal: 5, vertical: 5),
                      ...
                      child: DropdownButtonFormField(
                        hint: Text("Select Service",style:TextStyle(color: Colors.blue) ,) ,
                        items: categoryItems,
                        isExpanded: true,
                        decoration: InputDecoration(
                        ...
                        onChanged: (currentValue){
                          ...
                          ServiceIDController.text = currentValue;

                        },
                        value: ServiceIDController.text,
                      ),
                    );
                  }
                }),
